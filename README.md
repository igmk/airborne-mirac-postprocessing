...
- Raw_rpg to raw: creating one file per flight/chirp setting and cleaning data
- Raw to lev_0: artifact correction
- lev_0 to lev_1a: time match of instruments, coordinate transformation
...
- prepare_gps_ins.py: Reduction of gps_ins data to flight time, exclusion of measurement errors, and removal of some parameters that are not needed.
- prepare_raw_data.py: Combines the raw radar files listed in files_raw.yaml into a single file per flight. Additionally, the time attributes are changed to make them conform with CF conventions (xarray then converts to datetime automatically), the data is sorted along the time dimension and duplicate times are removed (keep first), and data is reduced to flight time, -999 replaced by nan (here also the nc attributes are not following CF conventions by having missing_value or _FillValue set to -999!). Finally, erroneous reflectivities are replaced by nan. Which occur during mosaic-aca.
...
- Raw data was created by Mario using the Matlab version of the RPG processing routines with the binary files as input
- The processing scripts do not change the data, they only convert it to netcdf and make some calculations such as Ze, ...
- All files are created with the same version of the RPG routines: https://github.com/igmk/w-radar/releases/tag/v2.0_202007
- The files used from the output end with *_ZEN_v2.0.nc
- The compact files do not contain all the required variables for the processing
...
- The measurement error is removed by hand. The basis is the script view_raw_ze.py. It creates 30-minute quicklooks of the measured Ze and the (afterwards) filtered Ze together with the measurement interval
- The measurement interval is about >8 s during 89 GHz calibration, while it is 1-2 seconds during all other times
- The times, when internal calibration is over and when the individual files end (times.txt, also printed by the view_raw_ze.py script), serve as bthe asis for the detection of erroneous Ze values.
...
- In the upper chirp from 0 to 600 m range, multiple surface reflections occur during several MOSAiC-ACA and HALO-AC3 flights. Mostly, this signal is clearly separated from clouds. However, it can also be seen within clouds, but no attempt was made to subtract the signal since both magnitude and vertical position could not be predicted. Therefore, only cases where the signal is clearly visible were removed by hand. In CFAD plots, the signal can be clearly seen, with similar Ze than cloud edges
- Time/height regions are identified using quicklooks of the raw signal with view_raw_ze.p,y similarly to the step above (errors 1). The script also plots identified regions, which are stored in surface_reflection_times_heights.yaml
...
- Filename convention: first 13 letters contain the date and initial time
- If flights are on the same day, the files need to be separated manually!! See the first lines in file_list_raw.py
...
- This step is done with the script prepare_raw_data.py, and it includes
  ...
  - adaptation of attributes (time! Default is not recognized by xarray and therefore changed)
...
- Create the calibration_times_empty.yaml file with a specific time interval
  ...
  - Calibrated parameters are: orientation of mirac (azimuth, zenith), and time offset (between the attitude/position sensor)
...
- If some calibration parameters are clearly wrong, because the ground signal is lost (maximum reflectivity not at surface, …), then adapt the time range for these flights (check quicklook, etc.). To calibrate single flights and not the entire campaign again, follow the steps (i.e., line in bash scripts above):
  ...
  - 4: call calibration_parameters.py to view the updated parameters
...
- Since this step takes a long time and some flights might be processed again, one setup file is created for each research flight in advance. The only difference is the campaign name and the time between the flights, such that only one flight is processed
...
- Format for PANGAEA:
  - time: radar time (gps/ins are shifted to this time), when the column was overflown
...
- One additional correction has to be made for MOSAiC-ACA flights for times when measurement errors due to 89 GHz calibration occur. Since the entire range profiles had to be removed, there is a risk that these flagged values (set to nan, while no cloud is also nan) are shifted into a column without measurement errors, leading to a reduced hydrometeor fraction. Therefore, all values in the transformed lev_3 data (height instead of range) that occur 30 seconds before a measurement error due to calibration are also set to nan and masked. These columns might otherwise not be entirely filled. The value of 30 seconds was chosen as a good approximation for high-altitude flight legs: the measurement of a full column takes 20 seconds at 3 km with 70 m/s and 25° viewing angle: tan(25)*3000/70 = 20 s
